...
def mine():
    ...
    required = ['proof']
    # 400 will reject the next request that comes with the same structure
    if not all(k in values for k in required):
        return jsonify({'message': 'Missing Values'}), 400

    # Validate the proof
    if not blockchain.valid_proof(last_proof, values.get('proof')):
        return jsonify({'message': 'Proof is not valid'}), 200
    ...
    block = blockchain.new_block(values.get('proof'), previous_hash)
